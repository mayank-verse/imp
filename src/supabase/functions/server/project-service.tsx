...
export class ProjectService {
  constructor() {
    // Remove circular dependency - AuthService will be passed when needed
  }
  ...
  async getManagerProjectsWithCredits(managerId: string): Promise<any[]> {
    console.log('Fetching projects with credits for manager:', managerId);
    
    const projects = await DatabaseRepository.getProjectsByManager(managerId);
    
    // For each project, get associated MRV data and carbon credits
    const projectsWithCredits = await Promise.all(
      projects.map(async (project) => {
        const mrvData = await DatabaseRepository.getMRVDataByProject(project.id);
        const credits = await DatabaseRepository.getAvailableCredits();
        const projectCredits = credits.filter(credit => credit.projectId === project.id);
        
        const totalCreditsGenerated = projectCredits.reduce((sum, credit) => sum + credit.carbonCredits, 0);
        const availableCredits = projectCredits.reduce((sum, credit) => sum + (credit.availableAmount || 0), 0);
        ...
        return {
          ...
          creditRecords: projectCredits
        };
      })
    );
    
    console.log('Manager projects with credits found:', projectsWithCredits.length);
    
    return projectsWithCredits;
  }

  async getAllProjects(authService?: AuthService): Promise<Project[]> {
    const projects = await DatabaseRepository.getAllProjects();

    // If no authService provided, return projects without manager details
    if (!authService) {
    ...
    const projectsWithManagers = await Promise.all(
      projects.map(async (project) => {
        try {
          const manager = await authService.getUserById(project.managerId);

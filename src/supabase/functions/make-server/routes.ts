...
import { MRVService } from "./mrv-service.ts";
  ...
  app.get("/make-server-a82c4acb/credits/marketplace", async (c) => {
    try {
      ...
      const enhancedCredits = availableCredits.map(credit => ({
        ...credit,
        pricePerCredit: credit.pricePerCredit || 25,
        projectName: credit.projectName || `Blue Carbon Project ${credit.projectId.slice(-8)}`,
        projectLocation: credit.projectLocation || 'Indian Coastal Region',
        ecosystemType: credit.ecosystemType || 'Mangrove',
    ...
    try {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        priceHistory.push({
          ...
          price: 25 + (Math.random() - 0.5) * 5, // Price varies around $25

...
export function BuyerDashboard({ user }: BuyerDashboardProps) {
  ...
  const fetchUserBalance = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a82c4acb/credits/balance`,
        {
          headers: {
  ...
  const fetchRetirementHistory = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a82c4acb/credits/retirements`,
        {
          headers: {
            Authorization: `Bearer ${session.access_token}`,
          },
  ...
  const handleRetirement = async () => {
    if (!selectedCredit || purchaseAmount <= 0 || !retirementReason.trim())
      return;
    ...
    try {
      ...
      const data = await response.json();
      toast.success(
        `Successfully retired ${purchaseAmount} tCO₂e credits! Permanent record created on blockchain.`
      );
  ...
  const handlePurchaseInit = async (credit: CarbonCredit) => {
    setSelectedCredit(credit);
    setPurchaseAmount(Math.min(credit.carbonCredits, 50));
    setShowPurchaseDialog(true); // Open the dialog immediately

    const quantity = 1; // Assuming a fixed quantity of 1 for a simplified flow
    const pricePerCredit = 1250; // Hardcoded or fetch the actual price
    const totalAmountInPaise = quantity * pricePerCredit * 100; // Example: 1250 * 100 = 125000 paise

    const {
      data: { session },
    } = await supabase.auth.getSession();
    if (!session) {
      toast.error("Please sign in to initiate payment.");
      return;
    }

    setIsOrderCreating(true);
    setRazorpayOrderId(""); // Clear previous ID
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a82c4acb/payments/create-order`,
        {
          ...
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${session.access_token}`, // Authentication for the Supabase Function
          },
          body: JSON.stringify({
            creditId: credit.id,
            quantity: quantity,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || "Server failed to create Razorpay Order."
        );
      }

      const data = await response.json();

      // This log will confirm the successful response from the Supabase function
      console.log("✅ Fetched Razorpay Order Details:", data);

      // Update state with the fetched Order ID and amount from the backend
      setRazorpayOrderId(data.orderId);
      setPurchaseAmount(data.amount); // Ensure the amount used is the verified amount from the backend
    } catch (error) {
      console.error("Order Initialization Error:", error);
      toast.error(`Payment initialization failed: ${error.message}`);
      setShowPurchaseDialog(false); // Close dialog on failure
    } finally {
      setIsOrderCreating(false);
    }
  };

  // Update existing openPurchaseDialog to use the new handler
  const openPurchaseDialog = (credit: CarbonCredit) => {
    handlePurchaseInit(credit);
  };
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600">
            Purchase and retire verified blue carbon credits with secure fiat
            payments
          </p>
        </div>
      </div>
          ...
          <div className="space-y-6">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-blue-900 mb-1">
                      Fiat-Anchored Registry
                    </h3>
                    <p className="text-sm text-blue-800">
                      Purchase credits with traditional payment methods (credit
                      card, bank transfer) while maintaining blockchain
                      transparency. Payments are instantly transferred to
                      project managers after platform fees.
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-purple-700">N/A</div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-blue-700">
                    {availableCredits
                      .reduce((sum, credit) => sum + credit.carbonCredits, 0)
                      .toLocaleString()}
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-teal-700">
                    {retirements
                      .reduce((sum, retirement) => sum + retirement.amount, 0)
                      .toLocaleString()}
                  </div>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="flex space-x-2">
                            <Button
                              ...
                              size="sm"
                              className="flex-1"
                              onClick={() => openPurchaseDialog(credit)} // Already calls the new handler from Step 2
                            >
          ...
          <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-green-800">
                      Manage and retire your purchased carbon credits
                    </p>
                  </div>
                  <div className="text-center">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Credits you have purchased grouped by project - retire them to
              ...
              <CardContent>
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm mt-2">
                    Visit the marketplace to purchase your first carbon credits
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            {retirements.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    View your carbon credit retirement certificates and offset
                    impact
                  </CardDescription>
                </CardHeader>
                <CardContent>
        ...
        <DialogContent>
          ...
          {isOrderCreating ? (
            <div className="text-center p-8">
              ...
              <p className="text-gray-600">
                Securely creating payment order...
              </p>
            </div>
          ) : selectedCredit && razorpayOrderId ? ( // Only render PaymentForm if Order ID is available
            <PaymentForm
              amount={purchaseAmount} // The verified amount from the backend response
              orderId={razorpayOrderId} // The CRITICAL missing piece!
              onSuccess={() => {
            ...
            />
          ) : selectedCredit && !razorpayOrderId && !isOrderCreating ? (
            <div className="text-center p-8 text-red-600">
              <p>Failed to initialize payment. Check console for API errors.</p>
            </div>
          ) : null}
        </DialogContent>
      </Dialog>
          ...
          <div className="space-y-4">
            <div className="bg-green-50 rounded-lg p-4">
              <p className="text-sm text-green-800">
                ...
                {userBalance.toLocaleString()} tCO₂e credits
              </p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="retirement-amount">
                Amount to Retire (tCO₂e)
              </Label>
              <Input
            ...
            <div className="bg-red-50 rounded-lg p-4 text-sm">
              ...
              <p className="text-red-700 mt-1">
                Retired credits cannot be transferred or sold. This action is
                permanent and will be recorded on the blockchain.
              </p>
            </div>
            ...
            <div className="flex justify-end space-x-2 pt-4">
              <Button
                variant="outline"
                onClick={() => setShowRetirementDialog(false)}
              >
                Cancel
              </Button>
              <Button
                ...
                disabled={
                  purchaseAmount <= 0 || !retirementReason.trim() || retiring
                }

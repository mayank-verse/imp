...
export function BuyerDashboard({ user }: BuyerDashboardProps) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // Get project manager info for this credit
      const projectManagerInfo = await mockPaymentService.getProjectManagerInfo(selectedCredit.projectId);
      if (!projectManagerInfo) {
        throw new Error('Project manager information not found');
      }

      // Process payment through mock payment service
      toast.info('Processing payment...');
      
      const paymentTransaction = await mockPaymentService.processPayment(
        {
          id: user.id,
        ...
        projectManagerInfo,
        {
          id: selectedCredit.projectId,
          name: `Project ${selectedCredit.projectId.slice(-8)}`
        },
        {
      ...
      if (paymentTransaction.status === 'completed') {
        // Now call the existing fiat purchase endpoint for blockchain recording
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-a82c4acb/credits/purchase-fiat`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(error.error || 'Failed to record purchase on blockchain');
        }

        toast.success(
          `Payment successful! $${paymentTransaction.totalAmount.toFixed(2)} charged to your card. ` +
          `$${paymentTransaction.netAmount.toFixed(2)} transferred to project manager. ` +
          `${purchaseAmount} tCO₂e credits added to your account.`
        );
      ...
      toast.error(`Failed to purchase credits: ${error}`);
    } finally {
      setPurchasing(false);
    }
  };
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-purple-700">
                    ${paymentTransactions.filter(t => t.status === 'completed').reduce((sum, t) => sum + t.totalAmount, 0).toFixed(0)}
                  </div>
                  <p className="text-xs text-purple-600 mt-1">
                    Across {paymentTransactions.filter(t => t.status === 'completed').length} transactions
                  </p>
                </CardContent>
              </Card>
            ...
            {paymentTransactions.length > 0 && (
              <Card>
                ...
                <CardContent>
                    ...
                    {Object.entries(
                      paymentTransactions
                        ...
                        .reduce((acc, transaction) => {
                          const projectId = transaction.projectName;
                          if (!acc[projectId]) {
                            acc[projectId] = {
                              projectName: transaction.projectName,
                              projectManagerName: transaction.projectManagerName,
                              totalCredits: 0,
                              transactions: []
                            };
                          }
                          acc[projectId].totalCredits += transaction.amount;
                          acc[projectId].transactions.push(transaction);
                          return acc;
                        }, {} as Record<string, any>)
                    ).map(([projectId, projectData]) => (
                      <Card key={projectId} className="border-2 border-blue-100">
                        <CardContent className="p-6">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                <div className="bg-blue-50 rounded-lg p-3">
                                  <p className="text-xs text-blue-600">Total Purchased</p>
                                  <p className="text-lg font-bold text-blue-900">
                                    {projectData.totalCredits.toLocaleString()} tCO₂e
                                  </p>
                                </div>
                                <div className="bg-green-50 rounded-lg p-3">
                                  <p className="text-xs text-green-600">Available to Retire</p>
                                  <p className="text-lg font-bold text-green-900">
                                    {Math.min(projectData.totalCredits, userBalance).toLocaleString()} tCO₂e
                                  </p>
                                </div>
                                ...
                                <div className="bg-yellow-50 rounded-lg p-3">
                                  <p className="text-xs text-yellow-600">Total Spent</p>
                                  <p className="text-lg font-bold text-yellow-900">
                                    ${projectData.transactions.reduce((sum: number, t: any) => sum + t.totalAmount, 0).toFixed(0)}
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="mb-4">
                                ...
                                <div className="space-y-2 max-h-32 overflow-y-auto">
                                  {projectData.transactions.map((transaction: any) => (
                                    <div key={transaction.id} className="text-xs text-gray-600 flex justify-between items-center py-1 px-2 bg-gray-50 rounded">
                                      <span>
                                        {transaction.amount.toLocaleString()} tCO₂e • {formatDate(transaction.processedAt)}
                                      </span>
                            ...
                            <div className="ml-6">
                              <Button 
                                onClick={() => {
                                  // Create a mock credit object for retirement
                                  const mockCredit = {
                                    id: `project-${projectId}`,
                                    projectId: projectId,
                                    carbonCredits: projectData.totalCredits,
                                    healthScore: 0.85,
                                    evidenceCid: 'mock-cid',
                                    verifiedAt: new Date().toISOString()
                                  };
                                  setSelectedCredit(mockCredit);
                  ...
                  {paymentTransactions.filter(t => t.status === 'completed').length === 0 && (
                    <div className="text-center py-12 text-gray-500">
          ...
          {selectedCredit && (
            <div className="space-y-4">
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button onClick={handlePurchase} disabled={purchaseAmount <= 0 || purchasing}>
